...
<Datatable
	data={{
		inputs: inputs,
		join_statements: JSON.stringify([{ transaction: 'transaction' }]),
		search_queries: ['a.id|b.uuid|b.seller_username'],
		model: 'TradeCost',
		...
		columns: [
            ...
            { label: 'Transaction Date',  data: 'date', through: ['transaction']  },
            { label: 'Amount', data: 'amount' },
            { label: 'Type', data: 'cost_type' },
            { label: 'Remarks', data: 'remarks' },
			{ label: 'Transaction', data: 'uuid', through: ['transaction'] }
		]
	}}
/>
